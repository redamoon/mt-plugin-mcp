...
| `page_list` / `page_get` / `page_create` / `page_update` / `page_delete` / `page_preview` | 記事ツールとは別。フォルダは `folder_id`。作成の既定は下書き。公開後は `rebuild_page` |
...
| `tag_list` / `tag_rename` / `tag_delete` | rename/delete は取り消し不可 |
